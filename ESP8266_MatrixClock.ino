...
//*    ESP8266 MatrixClock                                                                                  *
//*********************************************************************************************************
// Modified by ARTEM KULCHENKO on 01.08.2022
...
const char* PW   = "myWifiPassword";      // "myWiFiPassword";

WiFiUDP ntpUDP;
NTPClient timeClient(ntpUDP, "ru.pool.ntp.org", 7200, 60000);
...
<p>
    <i class="fas fa-thumbs-up" style="color:#c70039;"></i> 
...
#define TZName     "EET-1EEST,M3.5.0,M10.5.0/3"                     // Kaliningrad
...
#define BRIGHTNESS_LOW   0     // values can be 0...15
#define BRIGHTNESS_HIGH   10     // values can be 0...15
...
String M_arr[12] = {"янв.", "фев.", "мар.", "апр.", "мая", "июня", "июля", "авг.", "сен.", "окт.", "ноя.", "дек."};
String WD_arr[7] = {"вс.,", "пн.,", "вт.,", "ср.,", "чт.,", "пт.,", "сб.,"};
...
void BrightnessCheck(){
  ...
  int sensorValue = analogRead(LDRPIN); // Read sensor
...
/*  if (sensorValue > 500) {max7219_set_brightness(BRIGHTNESS_LOW);}
  ...
  max7219_set_brightness(sensorValue / 200);
}
...
/*
  ...
  String dataNTP = timeClient.getFormattedDate();

  int se = timeNTP.substring(6).toInt();                 //
  int mi = timeNTP.substring(3, 5).toInt();
  int ho = timeNTP.substring(0, 2).toInt();
  int ye = dataNTP.substring(0, 4).toInt();
  int mo = dataNTP.substring(5, 7).toInt();
  int da = dataNTP.substring(8, 10).toInt();
*/
...
void setup()
{
  ...
  if (!MDNS.begin("MatrixClock")) {             // Start the mDNS responder for esp8266.local
  ...
  server.on("/feelslike", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", String(feelslike).c_str());
  });
...
Serial.println();*/
    ...
    if (now.dayOfTheWeek() != 0){
      switch(nw){
      case 64000: mp3.volume(30); mp3.playMp3Folder(1); break;
      case 65900: mp3.volume(20); mp3.playMp3Folder(6); break;
      case 72000: mp3.volume(30); mp3.playMp3Folder(7); break;
      case 73000: mp3.volume(20); mp3.playMp3Folder(8); break;
      case 74000: mp3.volume(20); mp3.playMp3Folder(9); break;
      case 74700: mp3.volume(20); mp3.playMp3Folder(10); break;
      case 75200: mp3.volume(20); mp3.playMp3Folder(5); break;
      //case 220700: mp3.volume(20); mp3.playMp3Folder(5); break;
      default: break;
    }
  } // if (rtc.getweekday() != 0)
  ...
  if ((uint32_t)millis() / 1000ul % 86400ul == 0ul){
    syncTime();
        ...
        if (_f_tckr1s == true)        // flag 1sek
        {
            ...
            min2 = (now.minute() / 10);
#ifdef FORMAT24H
            std1 = (now.hour() % 10);  // 24 hour format
            std2 = (now.hour() / 10);
#else
            uint8_t h = now.hour();    // convert to 12 hour format
            if(h>12) h-=12;
            std1 = (h%10);
            std2 = (h/10);
#endif //FORMAT24H
            ...
            std22 = std2;
            _f_tckr1s = false;
            if (now.second() == 45) f_scroll_x1 = true; // scroll ddmmyy
            if (now.second() == 30) BrightnessCheck();// update brightness
            if (now.second() == 15) TempCheck(); //check temperature
#ifdef UDTXT
            if (now.second() == 25) f_scroll_x2 = true; // scroll userdefined text
#endif //UDTXT
        } // end 1s
...
//          -------------------------------------
            if(f_scroll_x1){ // day month year
                String txt= "   ";
                txt += WD_arr[now.dayOfTheWeek()] + " ";
                txt += String(now.day()) + " ";
                txt += M_arr[now.month() - 1] + " ";
                //txt += "20" + String(now.year()) + "   ";
                txt += String(now.year()) + " г.   ";
                sctxtlen=scrolltext(_dPosX, txt);
            }
//          -------------------------------------
            if(f_scroll_x2){ // user defined text
#ifdef UDTXT
                sctxtlen=scrolltext(_dPosX, temp );
#endif //UDTXT
            }
//          -------------------------------------
...
// -----------------------------------------------
        ...
        Serial.print("*"); // не удалять. без неё не работает.
    }  //end while(true)
